...
# 큰 텍스트
{large}이 텍스트는 크게 표시됩니다 (1.5rem){/large}

# 매우 큰 텍스트  
{xlarge}이 텍스트는 매우 크게 표시됩니다 (2rem){/xlarge}
...
# 인라인 조합
일반 텍스트 중에서 {highlight}특정 부분만 강조{/highlight}하거나 {bold}굵게{/bold} 만들 수 있습니다.
```
...
{large}오늘 다룰 내용:{/large}

- {bold}중요한 개념 설명{/bold}
- {highlight}핵심 포인트 강조{/highlight}
- 실제 예제 시연
...
| 문법 | 설명 | 결과 |
|------|------|------|
| `{center}텍스트{/center}` | 텍스트 중앙 정렬 | 중앙 정렬된 텍스트 |
| `{large}텍스트{/large}` | 큰 텍스트 (1.5rem) | **큰 텍스트** |
| `{xlarge}텍스트{/xlarge}` | 매우 큰 텍스트 (2rem) | **매우 큰 텍스트** |
| `{highlight}텍스트{/highlight}` | 하이라이트 텍스트 | 🟢 강조된 텍스트 |
| `{bold}텍스트{/bold}` | 굵은 텍스트 | **굵은 텍스트** |
...
### 🔧 개선사항
- **전체 화면 모드 키보드 이벤트 수정**: 전체 화면에서 화살표 위/아래 키가 정상적으로 작동합니다
- **커스텀 텍스트 문법 추가**: `{center}`, `{large}`, `{xlarge}`, `{highlight}`, `{bold}` 문법 지원
